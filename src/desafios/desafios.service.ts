...
export class DesafiosService {
    ...
    async atribuirDesafioPartida(_id: string, atribuirDesafioPartidaDto: AtribuirDesafioPartidaDto): Promise<void> {
        ...
        // Verifica se o desafio foi encontrado
        if (!desafioEncontrado) {
            throw new BadRequestException(`Desafio ${_id} não cadastrado!`);
        }
    
        // Filtra os jogadores do desafio para verificar se o vencedor faz parte do desafio
        const jogadorFilter = desafioEncontrado.jogadores.filter(jogador => jogador._id == atribuirDesafioPartidaDto.def);
    
        // Se o jogador não estiver no desafio, lança uma exceção
        if (jogadorFilter.length === 0) {
            throw new BadRequestException(`O jogador vencedor não faz parte do desafio!`);
        }
    
        // Cria a partida com os dados necessários
        ...
        // Atualiza o status do desafio para REALIZADO
        desafioEncontrado.status = DesafioStatus.REALIZADO;
    
        // Atribui a partida criada ao desafio encontrado, convertendo para objeto puro
        desafioEncontrado.partida = resultado._id as any;
    
        // Tenta salvar o desafio atualizado
        try {
            await this.desafioModel.findOneAndUpdate({ _id }, { $set: desafioEncontrado.toObject() }).exec();
        } catch (error) {
            // Se houver um erro, remove a partida salva e lança uma exceção
